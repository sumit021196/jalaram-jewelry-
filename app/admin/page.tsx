    ...
    return (
        <div className="space-y-6 md:space-y-10 pb-20">
            <div className="flex flex-col gap-2">
                <h1 className="text-2xl md:text-4xl font-black tracking-tighter text-gray-900 uppercase">
            ...
            <div className="grid grid-cols-1 gap-6 md:gap-10 lg:grid-cols-5">
                {/* Recent Orders */}
                <div className="lg:col-span-3 bg-white rounded-3xl md:rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <table className="min-w-full">
                            <thead>
                                <tr className="bg-gray-50/50">
                                    <th className="px-5 md:px-8 py-4 text-left text-[9px] font-black text-gray-400 uppercase tracking-widest">Patron</th>
                                    <th className="px-5 md:px-8 py-4 text-left text-[9px] font-black text-gray-400 uppercase tracking-widest">Status</th>
                                    <th className="px-5 md:px-8 py-4 text-right text-[9px] font-black text-gray-400 uppercase tracking-widest">Investment</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {recentOrders?.map((order) => (
                                    <tr key={order.id} className="hover:bg-gray-50 transition-colors group">
                                        <td className="px-5 md:px-8 py-4 md:py-5 text-sm font-bold text-gray-900">{order.customer_name}</td>
                                        ...
                                        <td className="px-5 md:px-8 py-4 md:py-5 text-sm text-gray-900 text-right font-black">₹{order.total_amount.toLocaleString()}</td>
                                    </tr>
                                )) || (
                ...
                <div className="lg:col-span-2 bg-white rounded-3xl md:rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                    <div className="p-5 md:p-8 border-b border-gray-50 flex items-center justify-between">
                        <div className="flex items-center gap-3">
...
function StatCard({
    ...
    return (
        <div className={`relative overflow-hidden rounded-2xl md:rounded-[2rem] border p-4 md:p-6 transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 ${variant === 'luxury' ? 'bg-gray-900 border-gray-900' : 'bg-white border-gray-100'}`}>
            <div className="flex flex-col gap-4">
                ...
                <div>
                    ...
                    <div className="flex items-baseline gap-2">
                        <p className={`text-xl md:text-2xl font-black tracking-tighter ${variant === 'luxury' ? 'text-white' : 'text-gray-900'}`}>{value}</p>
                    </div>
